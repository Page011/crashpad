...
- **Clipboard**: a real clipboard manager. It keeps text, images and files across restarts, has pinned clips,
  ...
  minified JSON, and URL / Base64 encode or decode. Right-click an image clip → Copy text (offline Windows OCR).
  The pipette button picks a colour from anywhere on screen and copies its hex.
...
- **Shots**: your screenshots folder as big previews in a grid (scroll down) or a row (scroll across).
  Click to copy, right-click for Quick Look, drag to pull the file into any app. The ⋯ menu (or M) marks a
  screenshot up with a pen, highlighter, arrows and boxes, then copies it or saves a "(marked)" copy; Copy text
  reads the text out of it.
...
**Ctrl+K** opens a command palette that searches notes, clips, screenshots, reminders and the shelf at once, and
runs commands: `= 12*7` (calculator, Enter copies), `timer 10m tea`, `remind me to call mum tomorrow 5pm`,
`new note`, snip, pick a colour, switch tabs. Anything else goes to your Inbox note.
**Alt+Shift+C** (configurable, or off) is quick capture from anywhere: a one-line bar grows out of the island;
type and press Enter to add it to Inbox.md, or start with "remind" / "timer".
...
| Alt+Shift+C | anywhere | quick capture bar (configurable) |
| Ctrl+K | panel | command palette |
...
| type, ↑↓, Enter, Shift+Enter, Ctrl+P, Ctrl+T, Shift+Del | Clipboard | search, pick, paste, copy only, pin, transform, delete |
| Ctrl+N, Ctrl+Shift+L / 8 / 7, Ctrl+Shift+D, Ctrl+B/I/U | Notes | new note, checklist / bullets / numbers, draw, format |
| arrows, Enter, Space, M, T, O, R, S, Ctrl+V | Shots | pick, copy, Quick Look, mark up, copy text, open, reveal, snip, save pasted image |
| P H A R, 1–6, Ctrl+Z/Y, Ctrl+C, Ctrl+S, Esc | Mark up | pen, highlighter, arrow, box, colour, undo/redo, copy, save copy, close |
...
| digits/type, Enter, ↑↓, Space, Delete, M | Live | new timer, pause/resume, remove, mute |
